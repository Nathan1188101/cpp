...
vector<int> finalPrices(vector<int>& prices){
    ...
    // where j = the minimum index such that j > i and prices[j]  <= prices[i]
    // otherwise no discount at all

    // j is always TO THE RIGHT of i
    // meaning discount is always coming from future items further down the list
    // 

    // discount always comes from the right of current i index (j is to the right)
